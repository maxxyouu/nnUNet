...
class IntensityAugmentationTransform(ImageOnlyTransform):
    def __init__(self, p_per_channel:float = 1, linear_factor=0.5, window_size=20, random_slope=None):
        super().__init__()
    ...
    def intensity_augmentation(self, image_array: torch.Tensor, **params) -> torch.Tensor:
        ...
        # Add a random linear component if not specify
        slope = self.random_slope
        if not self.random_slope:
            slope = np.random.choice([-1, 1])
        remapping_curve = smoothed_noise + self.linear_factor * slope * np.arange(256)
...
class nnUNetTrainer_DefaultDAAndIA_PostiveSlope(nnUNetTrainer):
    ...
    def get_training_transforms(
        ...
        #NOTE: intensity augmentation is added here.
        transforms.append(RandomTransform(
            IntensityAugmentationTransform(random_slope=1),
            apply_probability=0.5 # NOTE: to be tuned
        ))
...
class nnUNetTrainer_DefaultDAAndIA_NegativeSlope(nnUNetTrainer):
